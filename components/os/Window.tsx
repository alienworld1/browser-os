...
const Window: React.FC<WindowProps> = ({ window }) => {
  const {
    ...
    updateWindowSize // Get the new function from context
  } = useWindowManager();
  ...
  const handleDragStop: DraggableEventHandler = (e, data) => {
    ...
    if (e.target instanceof Element && e.target.classList.contains('react-resizable-handle')) {
        return;
    }
  ...
  const handleFocus = (e: React.MouseEvent) => {
      ...
      if (e.target instanceof Element) {
        ...
        if (targetClasses.contains('window-control-button') || targetClasses.contains('react-resizable-handle')) {
            return;
        }
      }
              ...
              >
                  ...
                  <div className="window-title-bar bg-gradient-to-b from-blue-600 to-blue-800 text-white h-6 flex items-center justify-between px-2 cursor-move select-none flex-shrink-0">
                      <div className="flex items-center space-x-1 overflow-hidden">
                          <Image src={window.icon} alt="" width={14} height={14} className="opacity-90"/>
                          <span className="text-xs font-semibold truncate">{window.title}</span>
                      </div>
                  ...
                  {/* Window Content */}
                  <div className="flex-grow overflow-auto bg-white"> {/* Added bg-white here for contrast */}
