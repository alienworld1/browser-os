  ...
  return (
    <div className="w-full h-full bg-gray-100 flex flex-col">
      ...
      <div className="flex items-center p-1 bg-gray-200 border-b border-gray-300">
        ...
        <button className="px-2 py-0.5 text-sm text-gray-600 opacity-50 cursor-not-allowed">&lt;</button>
        <button className="px-2 py-0.5 text-sm text-gray-600 opacity-50 cursor-not-allowed">&gt;</button>
        <button className="px-2 py-0.5 text-sm text-gray-600 opacity-50 cursor-not-allowed">↻</button>
      ...
      <div className="flex-grow overflow-hidden">
        <iframe
          ...
          onError={() => console.warn(`Error loading or embedding ${currentUrl}`)}
        >
          <p>Your browser does not support iframes, or the content is blocked.</p>
        </iframe>
      </div>
      <div className="p-1 bg-gray-200 border-t border-gray-300 text-xs text-gray-600 truncate">
          {/* Status Bar Placeholder */}
          {`Loading: ${currentUrl}` }
          <span className='text-red-600'> (Note: Many sites block embedding)</span>
      </div>
    </div>
  );
};
